...
\usepackage{hyperref}
\usepackage{float}
\usepackage[top=1.5in, bottom=1.25in, left=1in, right=1in]{geometry}
...
\title{Modelling of Nanosecond time-scale Transient response of GaN LEDs}
...
\section{Abstract}
\begin{center}
	Light emitting diodes (LEDs) are now being explored as devices used to calibrate the scintillation counters in large neutrino detection experiments by emitting small pulses of photons in the optical range. However, in these experiments, the shortest photon pulses achieved have been $\approx 3ns$ with large inconsistencies\cite{Veledar_Paper}, until recently in which consistent $1ns$ pulses have been generated,\cite{Data} which has inspired this investigation into the underlying mechanisms.\\ 
	In this thesis, two simulations are produced of different models, to simulate the light emission of a Gallium Nitride blue LED being switched on and off with in the order of a few nanoseconds. The results predict an approximate $t^5$ power law for the number of photons emitted in the pulse, but do not match the experimental data in terms of this law, but do produce photon counts in a range that the experimental data falls within. 
\end{center}
\bigskip
...
Zachary Humphreys	&Signed: \includegraphics[scale=0.1]{Figures/Signature} & Date: 28/04/2018
	\end{tabular*}
\end{minipage}
\bigskip
\hrule
